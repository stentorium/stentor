 ...
 */
export interface SimpleResponse<T = string | ResponseOutput>
  extends Partial<Actionable> {
  /**
 ...
 */
export type ScheduledResponse<T = string | ResponseOutput> = Schedulable<
  SimpleResponse<T>
>;
/**
 * A response that is contextual, it knows about your past interactions.
 */
export type LastActiveResponse<T = string | ResponseOutput> = LastActive<
  SimpleResponse<T>
>;
/**
 * A response that is dependent on a slot value in the current request.
 */
export type SlotDependentResponse<T = string | ResponseOutput> = SlotDependable<
  SimpleResponse<T>
>;
/**
 * A response that is dependent on a value on user storage.
 */
export type StorageDependentResponse<
  T = string | ResponseOutput
> = StorageDependable<SimpleResponse<T>>;
/**
 * A response that is dependent on a value on the request.
 */
export type RequestDependentResponse<
  T = string | ResponseOutput
> = RequestDependable<SimpleResponse<T>>;
/**
 * A response that is dependent on a system state such as if the user has linked their account.
 */
export type SystemDependentResponse<
  T = string | ResponseOutput
> = SystemDependable<SimpleResponse<T>>;
/**
 * A response that is dependent on a particular JSON path matcher.
 */
export type JSONDependentResponse<T = string | ResponseOutput> = JSONDependable<
  SimpleResponse<T>
>;
