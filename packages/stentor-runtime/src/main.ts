...
export const main = async (
  ...
  log().info(
    `platform:${request.platform}|channel:${request.channel}|request-type:${request.type}|request-key:${keyFromRequest(
      request
    )}|userId:${request.userId}${request.isHealthCheck ? "|HEALTH_CHECK" : ""}`
  );
  ...
  // #0.6 Handle EventRequest by sending directly to eventService
  if (isEventRequest(request)) {
    ...
    callback(null, { status: "acknowledged" }, request);
    return;
  }
  ...
  try {
    ...
    if (hasSessionId(request)) {
      if (!request.sessionId) {
        throw new Error(
          `Session ID is undefined when attempting to create update / retreive session store. ${request.sessionId}`
        );
      }
  ...
  if (channel.nlu) {
    ...
    if (
      ...
      const queryProps: NLURequestProps = {
        userId,
        sessionId,
        locale,
        channel: request.channel,
  ...
  if (isIntentRequest(request)) {
    ...
    context.session.set(
      SESSION_STORAGE_SLOTS_KEY,
      combineRequestSlots(context.session.get(SESSION_STORAGE_SLOTS_KEY), request.slots)
    );
    // We also keep track of the most recent KB result if it exists.
    context.session.set(
      SESSION_STORAGE_KNOWLEDGE_BASE_RESULT,
      combineKnowledgeBaseResults(
        context.session.get(SESSION_STORAGE_KNOWLEDGE_BASE_RESULT),
        request.knowledgeBaseResult
      )
    );
  }
