import { addDays, addMonths, addWeeks, addYears, eachWeekendOfInterval, endOfMonth, endOfWeek, endOfYear, format, formatISO, parse, startOfMonth, startOfWeek, startOfYear } from "./date-utils-lite.js";
 ...
 */
export function isDateTime(slotValue) {
    return !!slotValue && (!!slotValue.date || !!slotValue.time);
}
/**
 ...
 * @param slotValue - Slot value to check
 */
export function isDateTimeRange(slotValue) {
    return !!slotValue && (!!slotValue.start || !!slotValue.end);
}
/**
 ...
 */
export function isISO8601(potential) {
    if (!potential) {
        return false;
    }
 ...
 */
export function isISO8601Range(potential) {
 ...
 * @returns
 */
export function isDateTimeRangeString(potential) {
 ...
 */
export function getDateTimeFrom(date, includeOnly) {
    let slotDateTime;
    let dateTime;
    ...
    if (isISO8601(dateTime)) {
        ...
        if (includeOnly !== "date") {
            ...
            if (timeResults) {
                slotDateTime = Object.assign(Object.assign({}, slotDateTime), { time: timeResults[1] });
            }
        }
    }
    return slotDateTime;
 ...
 * @returns
 */
export function getDateTimeRangeFrom(date) {
    let delimiter;
 ...
 * @public
 */
export function parseDate(parsable, returnOnly) {
    const dateTime = parseNaturalDate(parsable);
    if (!dateTime) {
        return undefined;
    }
 ...
 */
export function parseRelativeDate(relative, now = new Date()) {
    let dateTime;
