 ...
 */
export function toResponseOutput(input) {
    ...
    if (typeof input === "string") {
        ...
        responseOutput = Object.assign(Object.assign({}, input), { ssml: ssmlify(input.ssml), displayText: dessmlify(input.displayText) });
    }
 ...
 * @param type
 */
export function dateTimeToSpeech(value, type = "ssml") {
    if (!value) {
        return "";
    }
    ...
    const toParse = Object.assign({}, value);
    if (!toParse.date) {
 ...
 */
export function dateTimeRangeToSpeech(value, type = "ssml") {
    if (!value) {
        return "";
    }
 ...
 * @param type
 */
export function durationToSpeech(duration, type = "ssml") {
    if (!duration) {
        return "";
    }
    const plural = duration.amount > 1;
 ...
 * @returns
 */
export function slotValueToSpeech(value, type = "ssml") {
    let speech;
    switch (typeof value) {
        case "string":
            speech = value;
...
export function mergeSuggestions(existing, incoming) {
    ...
    const existingMap = existing.reduce((map, suggestion, index) => {
