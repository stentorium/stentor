 ...
 * @param response
 */
export function isValidSSML(response) {
    if (!response) {
        return false;
    }
    let result = false;
 ...
 *
 */
export function cleanInvalid(outputSpeech) {
    if (outputSpeech && typeof outputSpeech === "string" && outputSpeech.indexOf("&") > 0) {
        ...
        if (outputSpeech.trim().startsWith("<speak>")) {
            try {
                ...
                document.childrenNamed("audio").forEach((node) => {
 ...
 */
export function ssmlify(str, clean = true) {
    // fast return
    if (!str) {
        return "<speak></speak>";
    }
 ...
 *
 */
export function concatText(one, two, delimiter) {
    one = one ? one : "";
    two = two ? two : "";
    let spacing = " ";
 ...
 */
export function concatSSML(one, two, delimiter) {
    return ssmlify(concatText(dessmlify(one), dessmlify(two), delimiter));
}
function removeTagWithContent(speech, tag) {
    // Step 1
 ...
 */
export function removeTagsWithContent(speech, tags) {
    if (!speech) {
        return speech;
    }
    tags.forEach(tag => {
